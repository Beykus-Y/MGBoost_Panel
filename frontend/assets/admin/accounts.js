...
export function createAccountUi({adminFetch,getJson,html,renderHtml,showPage,toast}){
  ...
  async function loadDashboard(){const target=document.getElementById('account-dashboard');try{const data=await getJson('/admin/dashboard'),c=data.grace_campaign;const needsAttention=!!(data.health.error_reconcile||data.health.slot_state_mismatches||data.health.child_state_mismatches);const alertCard=n=>n?' stat-card--alert':'';renderHtml(target,html`${metadataWarning(data.presentation_metadata_available)}${timelineUi.queuesSection(data.queues)}${c?html`<section class="dashboard-section"><div class="section-heading"><div><span class="eyebrow">Переходный период</span><h2>${c.active?`День ${c.day_of_14} из 14 · осталось ${formatDuration(c.seconds_remaining)}`:'Переходный период завершён'}</h2></div><button data-action="show-page" data-page="migration">Открыть «Миграция / Grace»</button></div><div class="progress-track large-progress"><span style="width:${c.elapsed_percent}%"></span></div><div class="progress-legend"><span>Прошло: ${formatPercent(c.elapsed_percent)}</span><span>Осталось: ${formatPercent(c.remaining_percent)}</span><span>До: ${formatTimestamp(c.ends_at)}</span></div><div class="stats-grid">${summaryRatio('Аккаунты готовы',c.parent_ready,c.accounts_total)}${summaryRatio('Telegram',c.telegram.BOUND,c.accounts_total,`не привязано ${c.telegram.UNREGISTERED+c.telegram.PENDING_LINK}, проверка ${c.telegram.AMBIGUOUS}`)}${summaryRatio('Аккаунты с real-device migration',c.accounts_with_real_lineage,c.accounts_total,`без lineage ${c.accounts_without_real_lineage}`)}<div class="stat-card"><div class="stat-label">Реальных устройств мигрировало</div><div class="stat-value">${c.total_real_lineages}</div><div class="stat-sub">genesis/bootstrap слоты не входят</div></div></div></section>`:''}<section class="dashboard-section${needsAttention?' dashboard-section--alert':''}"><div class="section-heading"><div><span class="eyebrow">Состояние</span><h2>${needsAttention?'Требует внимания':'Локальное состояние согласовано'}</h2></div></div><div class="stats-grid"><div class="stat-card${alertCard(data.health.error_reconcile)}"><div class="stat-label">Ошибки сверки</div><div class="stat-value">${data.health.error_reconcile}</div></div><div class="stat-card${alertCard(data.health.resolver_errors_72h)}"><div class="stat-label">Ошибки resolver за 72 ч</div><div class="stat-value">${data.health.resolver_errors_72h}</div></div><div class="stat-card${alertCard(data.health.slot_state_mismatches+data.health.child_state_mismatches)}"><div class="stat-label">Расхождения desired / observed</div><div class="stat-value">${data.health.slot_state_mismatches+data.health.child_state_mismatches}</div><div class="stat-sub">slot + child</div></div><div class="stat-card clickable${alertCard(data.tickets.unanswered)}" data-action="show-page" data-page="tickets"><div class="stat-label">Тикеты</div><div class="stat-value">${data.tickets.open}</div><div class="stat-sub">без ответа ${data.tickets.unanswered}</div></div></div></section><section class="dashboard-section"><div class="section-heading"><div><span class="eyebrow">Истекают скоро</span><h2>Ближайшие подписки</h2></div></div><div class="expiry-buckets"><span>Сегодня ${data.expiring.buckets.today}</span><span>≤3 дней ${data.expiring.buckets.three_days}</span><span>≤7 дней ${data.expiring.buckets.seven_days}</span><span>≤30 дней ${data.expiring.buckets.thirty_days}</span></div>${data.expiring.accounts.length?html`<div class="compact-list">${data.expiring.accounts.slice(0,6).map(row=>html`<button class="compact-row" data-action="open-account" data-account-id="${row.id}"><span><strong>${row.label}</strong>${row.label!==row.primary_alias&&row.primary_alias?html`<small>${row.primary_alias}</small>`:''}</span><strong>${formatDuration(row.seconds_remaining)}</strong></button>`)}</div>`:html`<div class="empty-state">Нет ближайших истечений</div>`}</section>`);}catch(error){renderHtml(target,html`<div class="notice notice-amber">Дашборд аккаунтов временно недоступен</div>`);throw error;}}
