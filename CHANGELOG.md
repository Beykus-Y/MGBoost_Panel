...
- Подготовлен PH1-07 dependency-hardening rollout без обновления Marzban application/schema: immutable Marzban 0.8.4 base получает hash-pinned `python-multipart 0.0.32`, а MGBoost — изолированный hash-locked Python runtime с `aiohttp 3.14.3`/`aiogram 3.30.0`. Production cutover остаётся закрыт verification gate до masked pre/post comparison.
