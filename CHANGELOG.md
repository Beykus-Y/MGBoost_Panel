...
- Подготовлено PH1-06 containment-изменение: новые локальные subscription references хранятся как SHA-256 verifier, application paths/query и nginx sensitive routes не должны логировать raw bearer, а LK удаляет legacy `?token=` из browser URL и далее использует same-origin header. Реальные legacy token/URL не вращаются и продолжают работать до staged Phase 4 migration.
...
- Добавлены root-only AES-256 GnuPG encrypted online backups двух SQLite DB с обязательной isolated restore/checksum/quick-check проверкой, 90-дневной controlled retention, отдельным one-time encrypted quarantine snapshot на 180 дней и systemd timer. Production enablement выполняется только после quarantine/restore gate по `docs/PHASE1_RETENTION_AND_BACKUP.md`.
