...
- PH2-03 server-side implementation passed focused staging: Internal/Filin HMAC nonces now use a hashed durable SQLite CAS with TTL/capacity limits and fail closed on store outage; optional signed v2 mutation idempotency blocks completed, conflicting and crash-pending retries without storing raw keys or responses. Legacy v1 remains compatible and enforcement stays disabled until the external mutation caller adopts stable v2 operation IDs, so PH2-03 is not yet marked complete.
