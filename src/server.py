...
from .routes.admin_devices import (
    ...
    handle_device_recovery_apply,
    handle_device_recovery_preview,
...
_ROUTES = [
    ...
    ("GET",    re.compile(r"^/admin/accounts/(?P<account_id>\d{1,18})/devices/(?P<slot_number>\d{1,3})/recover$"),
     lambda h, account_id, slot_number: handle_device_recovery_preview(h, account_id, slot_number)),
    ("POST",   re.compile(r"^/admin/accounts/(?P<account_id>\d{1,18})/devices/(?P<slot_number>\d{1,3})/recover$"),
     lambda h, account_id, slot_number: handle_device_recovery_apply(h, account_id, slot_number)),
