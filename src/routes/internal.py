...
from ..legacy_contract import validate_renew_payload, validate_user_payload
...
def handle_internal_user_create(handler):
    ...
    try:
        ...
        payload = validate_user_payload(data, creating=True)
...
def handle_internal_user_renew(handler, username):
    ...
    try:
        data = json.loads(read_body(handler) or b"{}")
        payload = validate_renew_payload(data)
    except (json.JSONDecodeError, ValueError) as exc:
        error_response(handler, 400, str(exc))
        return
    ...
    with _lock_marzban_username(handler, username):
        try:
            user = _client.renew_user(username, payload, admin_token)
        except Exception as exc:
