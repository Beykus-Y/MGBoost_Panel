...
class StarsPurchaseStore:
    ...
    def create_invoice(self, *, telegram_id: int, plan_code: str, duration_days: int,
        ...
        with self._lock:
            try:
                ...
                if account is None:
                    raise StarsPurchaseError("no active canonical owner account for Telegram identity")
    ...
    def validate_invoice_for_checkout(self, invoice_id: int, telegram_id: int, *, now: int | None = None) -> dict:
        ...
        with self._lock:
            ...
            if not row or row["invoice_kind"] != "CANONICAL_PLAN":
    ...
    def _validate_snapshot_locked(self, row: dict) -> None:
        if row["invoice_kind"] != "CANONICAL_PLAN":
        ...
        with self._lock:
            try:
                ...
                self._conn.execute(
                    ...
                    "VALUES (?,?,?,?,?,?,?,?,?)",
                    (int(invoice_id), row["account_id"], charge_id, provider_charge_id, int(payer_telegram_id),
    ...
    def pending_invoices(self) -> list[dict]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT * FROM stars_invoices WHERE invoice_kind='CANONICAL_PLAN' AND status='paid' ORDER BY id"
            ).fetchall()
