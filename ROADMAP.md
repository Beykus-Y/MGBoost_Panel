...
**Implemented locally, production cutover pending:** application request/error logs redact `/sub/{token}` and all query values; LK accepts legacy query bookmarks once, removes the bearer from browser URL state and uses a same-origin header for subsequent API calls; subscription/LK responses set `no-store` and `no-referrer`; browser subscription external links use `noreferrer`. New `sub_requests`, `user_devices` and `hysteria_stats` keys use stable `sha256:<hex>` references. An explicit atomic migration removes existing raw local keys without rotating the Marzban bearer and preserves device identity/Hysteria counters. Added daily AES-256 GnuPG-encrypted SQLite backup/restore verification with 90-day controlled retention, a one-time encrypted 180-day quarantine tool, systemd units and the runbook `docs/PHASE1_RETENTION_AND_BACKUP.md`. Production inventory currently has 198 raw `sub_requests`, 71 raw `user_devices`, no Hysteria rows and no scheduled application DB backup; these are not changed until quarantine/restore and deployment gates pass.
**Current tests:** full regression `391 passed, 1 skipped`; targeted containment/legacy/LK/concurrency/subscription/real-GPG-backup tests included. `systemd-analyze verify` accepted backup units (only sandbox user-lookup socket warnings). Production prerequisite completed: exactly one encrypted quarantine and first encrypted DB backup both passed isolated restore; nginx sensitive logging, 30-day logrotate and journald retention are active. Remaining before `[x]`: git-based application cutover, atomic local DB verifier migration, encrypted timer enablement, canary scan and exact backward-compatibility gate.
