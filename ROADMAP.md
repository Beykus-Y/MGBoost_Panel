...
**Осталось до `[x]`:** a separate owner-approved production canary gate that actually exercises revoke/free/rebind against a real (throwaway, not the existing approved dormant canary unless separately authorized) production child, since this task's brief explicitly forbids touching the existing production canary/slot/child/shadow binding. Until that owner-approved destructive-canary evidence exists, this stays `[~]`; the dormant code/schema deploy itself carries zero production risk (nothing imports it, no lifecycle operation is ever created automatically).
