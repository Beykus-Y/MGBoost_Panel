...
**Tests/gates:** focused child/broker/staging-guard tests `37 passed`; full regression `518 passed, 3 skipped`. The typed credential reread verifies both VLESS UUID and Shadowsocks password against hash-only local verifiers and persists neither raw value. A disposable current production DB copy applied the prerequisite migration twice (`true/false`), preserved all 38 prior table digests and 71/71 legacy device/HWID rows, left every alias/child/outbox table empty, and passed quick/FK checks. The 2026-08-25 isolated official Marzban 0.8.4 gate is **FAIL** before child creation: production-equivalent topology has 25 VLESS and zero Shadowsocks inbound, while Marzban refuses create/update of a user retaining the source's disabled Shadowsocks proxy (`HTTP 400`). Adding a staging SS inbound changes effective contract from 25/0 to 25/1 and is not accepted as proof. Real idempotency/lost-ACK therefore remains unverified outside unit contracts.
...
**Осталось до `[x]`:** resolve the disabled-Shadowsocks Marzban API incompatibility by an explicit approved architecture choice; obtain a full isolated real-Marzban PASS for exact 25/0 source/child shape, idempotency and lost-ACK; then dormant production schema/code gate, protected slot-HMAC and primary-admin mapping configuration, one parent/alias/slot/outbox canary mutation, worker dispatch/reread/reconcile and post-effect verification. The exact manifest is approved, but approval does not waive the failed technical gate. PH3-04 remains off. Contract/runbook: `docs/PHASE3_CHILD_PROVISIONING.md`.
...
23. Real Marzban 0.8.4 staging подтвердил конфликт пункта 22 с live topology: при 25 VLESS/0 Shadowsocks inbound публичный API запрещает создать или дополнить user с Shadowsocks proxy; добавление SS inbound меняет effective set на 25/1. PH3-03 production mutation заблокирована до явно утверждённого contract/topology/API решения и полного повторного staging PASS; прямой Marzban DB write не считается допустимым обходом.
