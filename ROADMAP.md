...
**Not done before this deploy — see the production entry below for what happened after owner approval:** as of the staging gate above, no `mgboost_shadow_resolver_bindings` row existed anywhere outside test/staging fixtures, and `MARZBAN_BROKER_RESOLVER_AUTH_KEY`/`SHADOW_RESOLVER_ENABLED` were unset in every deployed environment.
