...
**Осталось до `[x]`:** integrate the durable outbox worker and lazy child creation with the future account-aware resolver, prove multi-slot/concurrent production-like acceptance beyond the single fixed canary, expose ephemeral credential reread only to that resolver, and complete operational retry/reconciliation monitoring. No existing client may be switched until the separately approved Phase 4 bridge. PH3-04 remains off. Contracts/runbooks: `docs/PHASE3_CHILD_PROVISIONING.md`, `docs/SHADOWSOCKS_RETIREMENT.md`, `docs/PHASE3_DORMANT_PRODUCTION_CANARY.md`.
