...
**Production dormant deploy 2026-08-25:** see the cardinality/evidence block below; no reversible parent status/expiry transition was performed against production. A genuine ACTIVE->EXPIRED->RENEWED production canary would require a new throwaway parent (the existing reviewed `account_id=1` cannot itself be safely put through that cycle without touching its real PH3-03 child), which this task treats as a separate, explicit owner decision rather than taking on its own. **Verdict: `[~]`**, not `[x]` -- dormant implementation, full regression, and real isolated Marzban 0.8.4 destructive evidence are all complete; only the actual production reversible-transition canary remains, pending a separate owner-approved throwaway-parent gate.
