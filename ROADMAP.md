...
**PH2-01 status re-check after PH4-01 (2026-08-25):** the legacy alias bridge closure criterion is now satisfied by PH4-01 (`[~]`, see below). PH2-01 stays `[~]`, not `[x]`, because a genuinely separate, real criterion in this same entry's own "Accept/tests" line remains untested: "ordinary ownership rebind preserves token" and "compromise flow invalidates old opaque token and issues a new one" both describe behavior during a Telegram ownership rebind -- and PH2-05 (admin/user session and ownership lifecycle, the only place a rebind can happen) remains `[ ]`, not started. There is no rebind mechanism to test the opaque credential's behavior against yet. This is the sole remaining PH2-01 blocker; it is unrelated to PH4-01 and cannot be closed by any Phase 4 work.
...
**Valid legacy `/sub` proof:** not independently reproduced this session -- the raw production legacy token was not available to this session and was not extracted from logs/backups to obtain it (explicitly disallowed). Only the invalid-token 404 smoke check was performed, which the task's own instructions correctly do not treat as sufficient valid-token evidence. This is the sole reason PH4-01 is not marked `[x]`.
**Verdict: `[~]`.** Implementation, focused/full regression, and real isolated Marzban 0.8.4 destructive evidence are all complete and passing; dormant production deploy passed with zero real bridge activation. Only a genuine, safely-obtained valid production legacy `/sub` pre/post proof remains -- an owner-assisted gate (the owner retrieves and shares one real subscription URL/token through an already-authorized channel for a single pre/post comparison, never given to or stored by this session) is the exact next step to close this to `[x]`.
