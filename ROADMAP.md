...
**BUG-002 fix (2026-09-06, narrow bugfix session):** the "sold manual package omitted from enforcement" reachability named above is closed -- `WL_PACKAGE_SALES_ENABLED` (`src/config.py`, off by default) now fail-closed-gates manual package creation server-side (`src/wl_packages.py::assert_wl_package_sales_enabled`, called from `ManualPaymentStore.create_record`); BUG-002 reclassified `PREMATURELY_REACHABLE_PATH` -- packages were never a launched, owner-approved feature (see `BUGS.md` BUG-002). **This is not PH6-08 and does not change this entry's status:** refund finality RISK-016 and the general effective-quota enforcement story remain open; catalog/FIFO/rollover mechanics themselves were already correct and untouched.
...
**BUG-002 fix (2026-09-06, narrow bugfix session):** the "downstream WL authority diverges" component named above (a sold package reads a remainder that enforcement then ignores) can no longer be triggered by a *new* sale -- see `BUGS.md` BUG-002 (reclassified `PREMATURELY_REACHABLE_PATH`, fixed via `WL_PACKAGE_SALES_ENABLED`). **This entry stays `[~]`:** the entitlement engine's own `adjustment_state='NONE'`/`adjustment_bytes=0` contract is unchanged -- PH6-08's effective-quota/adjustment ledger still does not exist.
...
**BUG-002 fix (2026-09-06, narrow bugfix session):** confirms and hardens what this title's own Stars-side `sellable_catalog()` already did -- WL packages were never sellable here either way; the manual (RUB) admin channel's equivalent gap is now closed the same way (`WL_PACKAGE_SALES_ENABLED`, see `BUGS.md` BUG-002, reclassified `PREMATURELY_REACHABLE_PATH`). **This entry stays `[~]`:** BUG-004 (already fixed separately) and the general ledger-defect/rollout-state gaps this entry names are unaffected.
...
**BUG-002 fix (2026-09-06, narrow bugfix session):** the "paid pool agreement" gap named above (BUG-002: this module compares consumption to `base_quota_bytes` only, ignoring sold packages) can no longer be reached by a *new* package sale (see `BUGS.md` BUG-002, reclassified `PREMATURELY_REACHABLE_PATH`, fixed via `WL_PACKAGE_SALES_ENABLED`). **This entry stays `[~]` and `compute_parent_wl_pool` itself is unchanged:** it still does not consult package buckets at all -- that is PH6-08's own future scope, not built here.
...
**BUG-002 fix (2026-09-06, narrow bugfix session):** "actual decision ignores sold packages" named above is BUG-002; a *new* package sale reaching this gap is now closed off at the source (see `BUGS.md` BUG-002, reclassified `PREMATURELY_REACHABLE_PATH`, fixed via `WL_PACKAGE_SALES_ENABLED`). **This entry stays `[~]` and `wl_enforcement.py`/`decide_direction_from_pool` are unchanged:** they still decide only from the base-quota pool -- package-aware enforcement is PH6-08's own future scope. All-node access verification remains separately unavailable.
...
**BUG-002 fix (2026-09-06, narrow bugfix session) -- explicitly NOT closing this entry:** BUG-002 was reclassified `PREMATURELY_REACHABLE_PATH` and its reachability closed by a readiness gate (`WL_PACKAGE_SALES_ENABLED`, see `BUGS.md` BUG-002) -- that is a narrow fail-closed fix over the *manual sales channel*, not an implementation of this item. This entry's own scope (a durable GB adjustment/compensation ledger and its integration into WL enforcement) remains entirely absent and unbuilt; `[~]` is unchanged and correct. The task's own explicit instruction was "не реализуй PH6-08 целиком," and nothing here does.
...
**Step 1 implemented 2026-08-31:** logging/redaction hardening -- every `logger.*`/`print()` call across `src/stars.py`, `src/bot_monitor.py`, `src/database.py`, `src/bot_support.py` now interpolates `type(e).__name__` instead of the raw exception object (26 call sites); a repo-wide lint test (`tests/test_ops_observability_redaction.py`) makes this a permanent regression fixture. Structural + behavioral redaction proof for the pre-existing `backlog_snapshot()`/`usage_freshness()` read models, and a fail-open baseline proof (`AdminLoginRateLimiter`/`SubscriptionRateLimiter` never touch sqlite on their hot path) established before any new PH8-04 counters/tables exist.
...
| PH5-03 | PARTIAL | src/wl_package_catalog.py; src/wl_packages.py; src/wl_package_schema.py | test_wl_packages.py; BUG-002 harness | Catalog/FIFO/rollover/refund primitive exists; sold manual package omitted from enforcement, refund finality RISK-016. BUG-002 fix (2026-09-06): reachability closed via WL_PACKAGE_SALES_ENABLED, reclassified PREMATURELY_REACHABLE_PATH; RISK-016/PH6-08 gaps remain. |
| PH5-04 | PARTIAL | src/entitlement_engine.py::calculate_effective_entitlement | test_entitlement_engine.py; BUG-002 harness | Read composition exists; add-on/adjustment outputs explicitly NONE, downstream WL authority diverges. PH5-07 remains deferred, PH6-08 needed for complete adjustment inputs. BUG-002 fix (2026-09-06) closes new-sale reachability; adjustment_state='NONE' contract (PH6-08 absent) unchanged. |
...
| PH6-04 | PARTIAL | src/wl_parent_pool.py::compute_parent_wl_pool | test_wl_parent_pool.py; BUG-002 harness | Base accounting exists and retains historic child usage; paid pool agreement and all-consumer correctness incomplete. BUG-002 fix (2026-09-06) closes new-sale reachability; compute_parent_wl_pool itself still ignores packages (PH6-08 scope). |
...
| PH6-06 | PARTIAL | src/wl_enforcement.py; src/broker_operations.py; src/wl_enforcement_schema.py | test_wl_enforcement.py; BUG-002 harness | Epoch/manifest/minimal inbound update exists; actual decision ignores sold packages, all-node access verification unavailable. BUG-002 fix (2026-09-06) closes new-sale reachability; enforcement decision itself still base-only (PH6-08 scope). |
...
| PH6-08 | PARTIAL | src/wl_packages.py; src/entitlement_engine.py; src/wl_enforcement.py | test_wl_packages.py; test_entitlement_engine.py | STALE [ ]: package buckets/read breakdown implemented; general GB adjustment/compensation ledger and enforcement integration absent (BUG-002). BUG-002 fix (2026-09-06) is a fail-closed reachability gate over the manual sales channel only; does NOT implement this item's own adjustment ledger/enforcement integration. |
