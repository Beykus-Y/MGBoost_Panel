...
**Осталось -- one precise blocker:** real migration/device-rebind/revoke on a non-internal (DIRECT) account is not yet performed and is currently blocked, not merely pending scheduling. `resolve_account_device()` (shared PH2-01/PH4-01 tail) calls `parent_sync.refresh_desired_state()`, which requires an existing `mgboost_subscriptions` row (`CHECK(status='UNKNOWN_LEGACY' OR current_plan_version_id IS NOT NULL)`); `enroll_direct_account()` deliberately does not create one, because doing so would require declaring a `mgboost_plan_versions` device_limit/WL mode for an unproven historical tariff -- exactly the invented-catalog-tariff the owner explicitly forbade. Enabling a `mgboost_legacy_bridge_bindings` row for `cohort-2 account #3`/`cohort-2 account #4` before that subscription exists would not be a safe canary-only action: the bridge gate is username-level (`resolve_account_for_legacy_username`), so the customer's own real device would hit the same missing-subscription path on its very next legacy request and receive a fail-closed `INTERNAL_ERROR` response instead of its legacy subscription (`INTERNAL_ERROR` is not a fall-through outcome) -- a real outage, not a contained canary risk. This is exactly the plan/device-capacity-preservation problem PH4-08 (`[ ]`, depends on "authoritative payment/admin evidence" -- now available via the owner attestation above) is scoped to solve, and it was not implemented this session (out of explicit scope). No bridge binding, no migration, no revoke, and no Telegram ownership rebind was attempted on either real account. Real PH2-05 ownership rebind proof continues to rely on the existing focused integration tests (`tests/test_ph4_03_migration_cohort_integration.py`) plus account 1's real production mechanism proof, per owner guidance not to mutate a real customer's Telegram identity solely to check a box. Metrics/support runbook still not drafted.
