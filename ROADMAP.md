...
**Осталось до `[x]`:** prove multi-slot/concurrent production-like acceptance beyond the single fixed canary, run the SHADOW resolver below through the mandatory real isolated Marzban 0.8.4 gate and a production SHADOW-only deployment window, then integrate the future real-traffic account-aware resolution path. No existing client may be switched until the separately approved Phase 4 bridge. PH3-04 remains off. Contracts/runbooks: `docs/PHASE3_CHILD_PROVISIONING.md`, `docs/PHASE3_CHILD_WORKER.md`, `docs/SHADOWSOCKS_RETIREMENT.md`, `docs/PHASE3_DORMANT_PRODUCTION_CANARY.md`.
...
**Not yet proven:** an owner-directed manual INCY subscription refresh (as opposed to the two organic ones observed) has not been separately requested or performed; a longer SHADOW observation window under real device traffic; and the actual child-config-in-INCY parser/display check on a **separate, non-production** test profile, which remains a distinct future gate requiring explicit owner action on the device itself.
